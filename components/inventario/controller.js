...
//reporte de todo el capital de los prdoductos existentes en la sucursal
function getCapitalInventario(idSucursal) {
    let idSu = ObjectId(idSucursal);
    if (!idSucursal)
        return Promise.reject({ message: "El id de la sucursal es necesaria" });
    idSu = ObjectId(idSucursal);
    return getCapitalInventarioDB(idSu);
}
